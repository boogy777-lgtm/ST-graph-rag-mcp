...
import { getRuntimeConfig, RuntimeClient } from "../statechart/runtimeClient";
...
export class LadderEditorProvider implements vscode.CustomTextEditorProvider {
	...
	private runtimeIoOperations = new Map<
		string,
		Map<string, RuntimeIoOperationState>
	>();
	...
	private readonly runtimeController = new RuntimeController({
		openPanel: () =>
			vscode.commands.executeCommand("trust-lsp.debug.openIoPanel"),
		openSettings: () =>
	...
	public async resolveCustomTextEditor(
		...
		webviewPanel.webview.onDidReceiveMessage(async (message) => {
			...
			if (isRuntimePanelWebviewMessage(message)) {
				await this.handleRuntimePanelMessage(
					docId,
					document,
					message,
					webviewPanel,
			...
			switch (message.type) {
				...
				case "programState":
					{
						const validatedProgram = tryValidateSchemaV2Program(
							message.program,
						);
						if (validatedProgram) {
	...
	private registerDebugIoBridge(): void {
		...
		this.context.subscriptions.push(
			vscode.debug.onDidStartDebugSession((session) => {
				...
				this.structuredTextSessions.set(
					structuredTextSessionKey(session),
					session,
				);
	...
	private setPendingRuntimeIoOperation(docId: string, targetRaw: string): void {
		...
		state.timer = setTimeout(() => {
			...
			if (session) {
				this.postRuntimePanelIoState(
					docId,
					session.webviewPanel,
					session.document,
				);
			}
	...
	private normalizeDebugIoEntry(
		value: unknown,
	): RuntimePanelIoEntry | undefined {
		if (!value || typeof value !== "object") {
			return undefined;
		}
	...
	private normalizeDebugIoState(
		value: unknown,
	): RuntimePanelIoState | undefined {
		if (!value || typeof value !== "object") {
			return undefined;
		}
	...
	private collectProgramIoAddresses(
		...
		const runtimeFbInstanceName =
			this.runtimeFbInstanceNameForDocument(document);
	...
	private async handleRuntimePanelMessage(
		...
		switch (message.type) {
			...
			case "releaseInput": {
				...
				this.applyRuntimeIoOperationSuccess(docId, document, target, "release");
				this.postRuntimePanelIoState(docId, webviewPanel, document);
				void this.requestStructuredTextIoState();
				return;
			}
	...
	private normalizeProgramForPersistence(
		...
		return {
			...program,
			schemaVersion: 2,
			networks: [...program.networks].sort(
	...
	private loadProgram(document: vscode.TextDocument): LadderProgram {
		...
		try {
			...
			vscode.window.showErrorMessage(
				`Failed to parse ladder program: ${error}`,
			);
	...
	private async runSimulation(
		...
		vscode.window.showInformationMessage(
			"🚀 Ladder simulation started (100ms scan cycle)",
		);
	}
	...
	private async runHardware(
		...
		vscode.window.showInformationMessage(
			"🔧 Ladder hardware execution started",
		);
	}
