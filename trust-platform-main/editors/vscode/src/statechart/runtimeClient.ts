...
export class RuntimeClient {
			...
			try {
				...
				if (this.endpoint.startsWith("tcp://")) {
					...
					this.socket = net.createConnection(socketPath, () => {
						console.log(
							"Connected to trust-runtime via Unix socket:",
							socketPath,
						);
						resolve();
					});
					...
					reject(
						new Error(`Invalid control endpoint format: ${this.endpoint}`),
					);
					return;
				}
	...
	async writeIo(address: string, value: any): Promise<void> {
		try {
			...
			const response = await this.sendRequest("io.write", {
				address,
	...
	async forceIo(address: string, value: any): Promise<void> {
		try {
			const formattedValue = this.formatValue(value);
			const response = await this.sendRequest("io.force", {
				address,
				value: formattedValue,
			});
