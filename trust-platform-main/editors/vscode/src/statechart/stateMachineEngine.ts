...
export class StateMachineEngine {
	...
	constructor(
		...
		this.currentState =
			this.config.initial || Object.keys(this.config.states)[0];
		this.mode = mode;
	...
	private scheduleStateTimers(stateName: string): void {
		...
		for (const [eventName, transition] of Object.entries(state.on)) {
			...
			if (afterDelay !== undefined && afterDelay > 0) {
				console.log(
					`⏰ Scheduling auto-transition: ${stateName} --[${eventName}]--> (after ${afterDelay}ms)`,
				);
				const timerId = setTimeout(() => {
					console.log(
						`🔥 Timer fired: ${eventName} on ${stateName} (${afterDelay}ms elapsed)`,
					);
					void this.sendEvent(eventName).catch((error) => {
						console.error(
							`Failed to handle auto-transition event ${eventName}:`,
							error,
						);
					});
				}, afterDelay);
	...
	private cancelStateTimers(stateName: string): void {
		...
		if (timersToCancel.length > 0) {
			console.log(
				`🛑 Cancelled ${timersToCancel.length} timer(s) for state: ${stateName}`,
			);
		}
	...
	async sendEvent(eventName: string): Promise<boolean> {
		...
		if (!transition) {
			console.log(
				`Event ${eventName} not available in state: ${this.currentState}`,
			);
			return false;
		}
		...
		console.log(
			`Transitioned from ${this.previousState} to ${this.currentState} via ${eventName}`,
		);
	...
	private async executeHardwareAction(actionName: string): Promise<void> {
		if (!this.config.actionMappings) {
			console.warn(
				`⚠️  No actionMappings defined, cannot execute: ${actionName}`,
			);
			return;
		}
		...
		if (!this.runtimeClient || !this.runtimeClient.isConnected()) {
			console.error(
				`❌ Runtime client not connected, cannot execute: ${actionName}`,
			);
			return;
		}
		...
		try {
			switch (mapping.action) {
				case "WRITE_OUTPUT":
					if (mapping.address !== undefined && mapping.value !== undefined) {
						console.log(
							`🔌 [HW] ${actionName} → FORCE ${mapping.value} on ${mapping.address}`,
						);
				...
				case "WRITE_VARIABLE":
					if (mapping.variable !== undefined && mapping.value !== undefined) {
						console.log(
							`🔌 [HW] ${actionName} → SET ${mapping.variable} = ${mapping.value}`,
						);
				...
				case "SET_MULTIPLE":
					if (mapping.targets) {
						console.log(
							`🔌 [HW] ${actionName} → FORCE MULTIPLE (${mapping.targets.length} outputs)`,
						);
			...
			console.error(
				`❌ Failed to execute hardware action ${actionName}:`,
				error,
			);
		}
	}
