...
import { BlocklyEngine, type BlocklyWorkspace } from "./blocklyEngine";
import { BlocklyInterpreter } from "./blocklyInterpreter";
import { getRuntimeConfig, RuntimeClient } from "./runtimeClient";
...
export class BlocklyEditorProvider implements vscode.CustomTextEditorProvider {
		...
		const messageSubscription = webviewPanel.webview.onDidReceiveMessage(
			(message) => {
				if (isRuntimePanelWebviewMessage(message)) {
					void this.handleRuntimePanelMessage(
						docId,
						document,
						message,
						webviewPanel,
	...
	private async generateCode(
		...
		try {
			...
			const saveCode = await vscode.window.showQuickPick(["Yes", "No"], {
				placeHolder: "Save generated ST code to file?",
			});

			if (saveCode === "Yes") {
				const stFileName = document.uri.fsPath.replace(
					/\.blockly\.json$/,
					".st",
				);
				...
				vscode.window.showInformationMessage(
					`ST code saved to ${path.basename(stFileName)}`,
				);
			}
		} catch (error) {
			const errorMessage =
	...
	private async startExecution(
		...
		// Start execution
		void this.executeBlocksWithHighlight(
			workspaceData,
			interpreter,
			webviewPanel,
		);
