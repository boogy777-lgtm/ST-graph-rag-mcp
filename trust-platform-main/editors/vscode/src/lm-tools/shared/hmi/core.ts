...
export function hmiSeverityRank(
	severity: HmiValidationCheck["severity"],
): number {
	if (severity === "error") {
		return 0;
	}
...
export function parseHmiSchemaPayload(
	value: unknown,
): HmiSchemaResult | undefined {
	...
	for (const item of widgetValues) {
		...
		widgets.push({
			...
			data_type:
				typeof widget.data_type === "string" ? widget.data_type : "UNKNOWN",
	...
	for (const item of pageValues) {
		...
		const sections = sectionsRaw
			...
			.filter(
				(
					entry,
				): entry is { title: string; span: number; widget_ids?: string[] } =>
					!!entry,
			);
		pages.push({
			id: page.id,
...
function parseHmiSceneBindings(
	value: unknown,
): HmiSchemaResult["pages"][number]["bind3d"] {
	...
	const bindings = value
		...
		.filter(
			(entry): entry is { node: string; property: string; source: string } =>
				!!entry,
		);
	return bindings.length > 0 ? bindings : undefined;
}

export function parseHmiValuesPayload(
	value: unknown,
): HmiValuesResult | undefined {
	const record = asRecord(value);
	if (!record) {
		return undefined;
	}
...
export function normalizeEvidenceRunId(
...
export function layoutDescriptorPages(
...
export function parseQuotedArrayFromToml(
	content: string,
	key: string,
): string[] {
...
export async function readHmiLayoutSnapshot(
	...
	const tomlFiles = entries
		.filter(
			([name, kind]) =>
				kind === vscode.FileType.File && isHmiTomlFileName(name),
		)
		.map(([name]) => name)
		.sort((left, right) => left.localeCompare(right));
	const svgFiles = entries
		.filter(
			([name, kind]) =>
				kind === vscode.FileType.File && name.toLowerCase().endsWith(".svg"),
		)
		.map(([name]) => name)
		.sort((left, right) => left.localeCompare(right));
...
export function isHmiPageTomlFileName(name: string): boolean {
	...
	return (
		isHmiTomlFileName(name) &&
		lower !== "_config.toml" &&
		!lower.endsWith(".view.toml")
	);
}
...
export function renderIntentToml(params: HmiPlanIntentParams): string {
	...
	lines.push(`goals = [${goals.map((value) => tomlQuote(value)).join(", ")}]`);
	lines.push(`kpis = [${kpis.map((value) => tomlQuote(value)).join(", ")}]`);
	lines.push(
...
export function parseWritePolicyFromConfigToml(
...
export function normalizeHmiBindingsCatalog(
	response: unknown,
): HmiBindingCatalog {
...
export function bindingFingerprint(
	entry: Omit<HmiLockEntry, "binding_fingerprint">,
): string {
	return crypto
		.createHash("sha256")
