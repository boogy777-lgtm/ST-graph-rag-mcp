...
import { type LspClientProvider, MAX_ITEMS } from "./types";
...
export function resolveLspRange(range: vscode.Range): {
	...
	return {
		start: resolveLspPosition(range.start),
		end: resolveLspPosition(range.end),
	};
}
...
export function toLspDiagnostic(diagnostic: vscode.Diagnostic): {
	...
	return {
		...
		relatedInformation: relatedInformation?.length
			? relatedInformation
			: undefined,
...
export function formatLspLocation(location: {
	uri: string;
	range: any;
}): string {
...
export function summarizeLspTextEdits(
	...
	edits: Array<{ range: string; newTextPreview: string }>;
	truncated: boolean;
} {
