...
const VISUAL_SUFFIXES: ReadonlyArray<{
...
export function generateVisualRuntimeEntrySource(
	...
	const ladderGlobals =
		sourceKind === "ladder" && typeof sourceText === "string"
			...
			: [];
	const runtimeGlobals = [`  ${fbInstanceName} : ${fbType};`, ...ladderGlobals];
...
function ladderRuntimeGlobalDeclarations(sourceText: string): string[] {
	...
	for (const variable of program.variables) {
		...
		declarations.push(`  ${name}${addressClause} : ${stType}${initialClause};`);
		emitted.add(name);
	}
...
export async function syncVisualCompanionFromUri(
	...
	try {
		...
		const stCode = generateVisualCompanionBody(
			sourceUri,
			sourceKind,
			sourceText,
		);
		const companionUri = await writeCompanionStFile(
			sourceUri,
			sourceKind,
			stCode,
		);
...
export async function openCompanionForVisualSource(
	...
	const companionDocument =
		await vscode.workspace.openTextDocument(companionUri);
