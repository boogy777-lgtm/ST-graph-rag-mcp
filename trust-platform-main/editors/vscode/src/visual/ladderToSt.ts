...
function defaultScope(
	scope: LadderProgram["variables"][number]["scope"],
): "local" | "global" {
	...
	if (!Array.isArray(variables)) {
		...
		for (let index = 0; index < variables.length; index += 1) {
			...
			if (
				...
				errors.push(
					`${prefix}.scope must be 'local' or 'global' when provided.`,
				);
			}
	...
	if (!Array.isArray(networks)) {
		...
		for (
				...
				for (
					...
					if (node.type === "timer") {
						...
						if (node.input !== undefined && !isString(node.input)) {
							errors.push(
								`${nodePrefix}.input must be a string when provided.`,
							);
						}
					...
					if (node.type === "counter") {
						if (!isCounterType(node.counterType)) {
							errors.push(
								`${nodePrefix}.counterType must be CTU, CTD, or CTUD.`,
							);
						}
						if (!isString(node.instance) || node.instance.trim().length === 0) {
							errors.push(`${nodePrefix}.instance must be a non-empty string.`);
			...
			if (!Array.isArray(network.edges)) {
				...
				for (
					let edgeIndex = 0;
					edgeIndex < network.edges.length;
					edgeIndex += 1
				) {
					...
					if (edge.points !== undefined) {
						if (!Array.isArray(edge.points)) {
							errors.push(
								`${edgePrefix}.points must be an array when provided.`,
							);
						} else {
							for (
								let pointIndex = 0;
...
function compareNodesDeterministic(
	left: LadderNode,
	right: LadderNode,
): number {
	...
	for (const network of orderedNetworks) {
		...
		for (const counter of counters) {
			...
			if (counter.counterType === "CTU") {
				lines.push(
					`${counterVar}(CU := ${inputExpr}, R := FALSE, PV := ${pv});`,
				);
			} else if (counter.counterType === "CTD") {
				lines.push(
					`${counterVar}(CD := ${inputExpr}, LD := FALSE, PV := ${pv});`,
				);
		...
		for (const compare of compares) {
			...
			validateDeclaredSymbol(
				left,
				`compare '${compare.id}' left operand`,
				declaredSymbols,
			);
			validateDeclaredSymbol(
				right,
		...
		for (const math of maths) {
			...
			validateDeclaredSymbol(
				left,
				`math '${math.id}' left operand`,
				declaredSymbols,
			);
			validateDeclaredSymbol(
				right,
				`math '${math.id}' right operand`,
				declaredSymbols,
			);
			...
			if (math.op === "DIV") {
				...
				lines.push(
					`${INDENT}${INDENT}${target} := (${left}) ${operator} (${right});`,
				);
