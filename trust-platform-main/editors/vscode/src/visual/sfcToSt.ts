...
function convertProgramToFunctionBlock(
	...
	const programHeaderIndex = lines.findIndex((line) =>
		/^\s*PROGRAM\b/i.test(line),
	);
	if (programHeaderIndex < 0) {
		throw new Error(
			"Generated SFC Structured Text does not contain a PROGRAM header.",
		);
	}
	...
	if (endProgramIndex < 0) {
		throw new Error(
			"Generated SFC Structured Text does not contain END_PROGRAM.",
		);
	}
