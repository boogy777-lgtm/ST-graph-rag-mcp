...
export function runtimePanelModeToUi(
	mode: "simulate" | "online",
): RuntimeUiMode {
	return mode === "online" ? "external" : "local";
}

export function runtimeUiModeToPanel(
	mode: RuntimeUiMode,
): "simulate" | "online" {
...
export function collectRuntimePanelSettings(
	...
	return {
		...
		debugAdapterEnv:
			section.get<Record<string, string>>("debug.adapter.env") ?? {},
		runtimeControlEndpoint:
			section.get<string>("runtime.controlEndpoint") ?? "",
		runtimeControlAuthToken:
			section.get<string>("runtime.controlAuthToken") ?? "",
