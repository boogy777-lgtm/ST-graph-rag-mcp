...
export function mountStRuntimePanel(
	...
	const collectSettingsPayload = (): SettingsPayload | null => {
		...
		return {
			...
			debugAdapterArgs: textToArray(
				getFieldValue(settingsFields.debugAdapterArgs),
			),
			debugAdapterEnv,
	...
	const applySettingsPayload = (payload: SettingsPayload) => {
		...
		setFieldValue(
			settingsFields.debugAdapterPath,
			payload.debugAdapterPath || "",
		);
		setFieldValue(
	...
	const renderDiagnostics = () => {
		...
		const targetLabel = compileState.target
			? fileLabel(compileState.target)
			: "";
	...
	const applyFilter = (entries: IoEntry[]): IoEntry[] => {
		...
		return (entries || []).filter((entry) => {
			const haystack =
				`${entry.name || ""} ${entry.address || ""}`.toLowerCase();
