...
function nearestProjectRoot(fsPath: string): string | undefined {
	let current =
		fs.existsSync(fsPath) && fs.statSync(fsPath).isDirectory()
			? fsPath
			: path.dirname(fsPath);
	while (true) {
...
export function registerStTestIntegration(
	context: vscode.ExtensionContext,
): void {
	...
	function updateResultState(
		...
		for (const testCase of payload.tests) {
			...
			const lineMap =
				stateByUriLine.get(uriKey) ?? new Map<number, LastResultEntry>();
	...
	function updateTestItemState(projectRoot: string): void {
		...
		for (const test of testById.values()) {
			...
			if (!outcome || outcome === "passed") {
				...
				item.error =
					runtimeCase.message?.trim() || `Test ${runtimeCase.status}`;
			}
		}
	}
	...
	const codeLensProvider: vscode.CodeLensProvider = {
		provideCodeLenses(document: vscode.TextDocument): vscode.CodeLens[] {
			...
			const discovered = discoverTestsFromText(
				document.uri,
				document.getText(),
				projectRoot,
			);
	...
	async function resolveSingleTarget(
		args?: RunOneArgs,
	): Promise<DiscoveredTest | undefined> {
		...
		if (uri && args?.name && args?.line && args?.kind) {
			...
			if (kind in TEST_KIND_SET) {
				const key = discoveredTestKey(
					uri.fsPath,
					args.line,
					kind as TestKind,
					args.name,
				);
				const existing = Array.from(testById.values()).find(
					(test) => test.key === key,
				);
				if (existing) {
					return existing;
	...
	async function runOne(
		target: DiscoveredTest,
	): Promise<RuntimeCase | undefined> {
		const payload = await executeRuntimeTests(
			context,
			target.projectRoot,
			target.name,
		);
			...
			async (args?: RunAllArgs): Promise<RuntimePayload | undefined> => {
				...
				if (!projectRoot) {
					vscode.window.showErrorMessage(
						"No project folder found for ST test run.",
					);
					return undefined;
				}
				try {
