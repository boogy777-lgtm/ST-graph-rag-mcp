...
import { SfcEngine, type SfcWorkspace } from "./sfcEngine";
...
export class SfcEditorProvider implements vscode.CustomTextEditorProvider {
			...
			(e) => {
				if (e.document.uri.toString() === document.uri.toString()) {
					...
					this.runtimeIoState.set(
						docId,
						this.buildIoStateFromDocument(document),
					);
	...
	private getHtmlForWebview(webview: vscode.Webview): string {
		const scriptUri = webview.asWebviewUri(
			vscode.Uri.file(
				path.join(this.context.extensionPath, "media", "sfcWebview.js"),
			),
		);
		const styleUri = webview.asWebviewUri(
			vscode.Uri.file(
				path.join(this.context.extensionPath, "media", "sfcWebview.css"),
			),
		);
	 ...
	 */
	private async handleToggleBreakpoint(
		docId: string,
		stepId: string,
	): Promise<void> {
		const state = this.activeExecutions.get(docId);
		...
		if (workspace.variables) {
			for (const variable of workspace.variables) {
				if (variable.address) {
					...
					const displayValue =
						value !== undefined ? boolToDisplay(Boolean(value)) : "FALSE";
