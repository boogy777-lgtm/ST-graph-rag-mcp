...
import type { ControlRequestHandler } from "./types";
...
export function createControlRequestSender(): ControlRequestHandler {
	...
	return async function sendControlRequest(
		...
		const parsed: ParsedControlEndpoint | undefined =
			parseControlEndpoint(endpoint);
		if (!parsed) {
					...
					if (line.length > 0) {
						try {
							...
							if (parsedLine.ok) {
								...
								const errorText =
									typeof parsedLine.error === "string" &&
									parsedLine.error.length > 0
										? parsedLine.error
