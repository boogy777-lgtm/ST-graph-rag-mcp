...
export async function layoutFileUri(
...
export function normalizeLayoutOverrides(value: unknown): LayoutOverrides {
	...
	for (const [widgetPath, rawOverride] of Object.entries(value)) {
		...
		if (
			typeof rawOverride.order === "number" &&
			Number.isFinite(rawOverride.order)
		) {
			normalized.order = rawOverride.order;
		}
		if (
			typeof rawOverride.min === "number" &&
			Number.isFinite(rawOverride.min)
		) {
			normalized.min = rawOverride.min;
		}
		if (
			typeof rawOverride.max === "number" &&
			Number.isFinite(rawOverride.max)
		) {
...
export function validateLayoutSavePayload(payload: unknown): LayoutOverrides {
	...
	for (const [widgetPath, override] of Object.entries(parsed)) {
		...
		if (
			...
			!/^[A-Za-z0-9._-]+$/.test(override.page)
		) {
			throw new Error(
				`page for '${widgetPath}' contains unsupported characters`,
			);
		}
	}
	return parsed;
...
export function applyLayoutOverrides(
	...
	const maxExistingOrder = schema.pages.reduce(
		(max, page) =>
			Math.max(max, Number.isFinite(page.order) ? page.order : max),
		0,
	);
