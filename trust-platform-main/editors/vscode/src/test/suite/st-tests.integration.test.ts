...
suite("ST test workflow integration (VS Code)", function () {
	...
	suiteSetup(async () => {
		...
		await vscode.workspace.fs.createDirectory(
			vscode.Uri.joinPath(projectRoot, "src"),
		);
	...
	test("discovers TEST_PROGRAM and TEST_FUNCTION_BLOCK declarations", async () => {
		...
		assert.ok(
			discovered && discovered.length === 3,
			"Expected 3 discovered ST tests.",
		);
		assert.ok(
			discovered?.some(
				(entry) => entry.name === "Pass_Case" && entry.kind === "TEST_PROGRAM",
			),
		...
		assert.ok(
			discovered?.some(
				(entry) =>
					entry.name === "Pass_Fb" && entry.kind === "TEST_FUNCTION_BLOCK",
			),
	...
	test("run all and run single commands execute expected tests", async () => {
		const all = (await vscode.commands.executeCommand("trust-lsp.test.runAll", {
	...
	test("state updates track pass/fail results for UI decorations", async () => {
		...
		assert.ok(
			...
			"Expected last-result state entries.",
		);
