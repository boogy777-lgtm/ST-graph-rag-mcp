...
function evaluateStBooleanExpression(
	...
	const consume = (expected?: string): string => {
		...
		if (!token) {
			throw new Error(
				"Unexpected end of expression while parsing ST boolean expression.",
			);
		}
...
suite("Visual ST companion generation", () => {
	test("maps visual source files to sibling .st companions", () => {
		const ladderUri = vscode.Uri.file(
			"/tmp/demo/simple-start-stop.ladder.json",
		);
		...
		assert.ok(
			companionStUriFor(sfcUri).fsPath.endsWith(
				path.join("tmp", "demo", "snake.st"),
			),
		);
		assert.ok(
	...
	test("generates runtime entry wrapper with configuration and program binding", () => {
		...
		assert.match(
			wrapper,
			/VAR_GLOBAL[\s\S]*fb_simple_start_stop : FB_simple_start_stop_LADDER;/,
		);
		...
		assert.doesNotMatch(
			wrapper,
			/PROGRAM PRG_simple_start_stop_VISUAL[\s\S]*\nVAR\n/,
		);
	});
	...
	test("generates ladder companion as function block", () => {
      ...
      "metadata": { "name": "simple-start-stop", "description": "test" }
		...
		const st = generateLadderCompanionFunctionBlock(
			program,
			"simple-start-stop",
		);
