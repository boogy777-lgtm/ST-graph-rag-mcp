...
suite("HMI preview integration (VS Code)", function () {
	...
	test("panel open + schema/value refresh pipeline", async () => {
		...
		assert.strictEqual(
			state.schema?.widgets.length,
			1,
			"Expected one widget in schema.",
		);

		await __testForcePollValues();
		const first = __testGetHmiPanelState().values?.values[widgetId]?.v as
			| boolean
			| undefined;
		await __testForcePollValues();
		const second = __testGetHmiPanelState().values?.values[widgetId]?.v as
			| boolean
			| undefined;
		...
		assert.notStrictEqual(
			first,
			second,
			"Expected value updates on subsequent poll.",
		);
	});
	...
	test("descriptor watcher refreshes open panel on hmi toml and svg changes", async () => {
		...
		const watchDir = vscode.Uri.joinPath(
			fixturesRoot,
			`watch-${Date.now()}`,
			"hmi",
		);
	...
	test("panel process page loads local svg asset and keeps bindings metadata", async () => {
		...
		try {
			...
			const state = __testGetHmiPanelState();
			const page = state.schema?.pages.find(
				(entry) => entry.id === "process",
			) as
			...
			assert.ok(
				typeof page?.svg_content === "string" &&
					page.svg_content.includes("<svg"),
	...
	test("LM HMI Phase 6 plan_intent writes deterministic _intent.toml", async () => {
		...
		const input = {
			...
			goals: [
				"Reduce alarm acknowledgement latency",
		...
		const written = Buffer.from(
			await vscode.workspace.fs.readFile(intentUri),
		).toString("utf8");
		assert.strictEqual(
			written,
	...
	test("LM HMI Phase 6 validate emits _lock.json evidence and prunes retention", async () => {
		...
		await vscode.workspace.fs.writeFile(
			...
			Buffer.from(["[write]", "enabled = false", ""].join("\n"), "utf8"),
		);
		await vscode.workspace.fs.writeFile(
		...
		assert.strictEqual(
			writePayload.ok,
			true,
			`validate payload: ${JSON.stringify(writePayload)}`,
		);
		assert.strictEqual(writePayload.dry_run, false);
		...
		const lockContent = Buffer.from(
			await vscode.workspace.fs.readFile(lockUri),
		).toString("utf8");
		const lockPayload = JSON.parse(lockContent);
		assert.strictEqual(lockPayload.version, 1);
		assert.ok(
			Array.isArray(lockPayload.widgets) && lockPayload.widgets.length >= 1,
		);
		assert.strictEqual(lockPayload.widgets[0].path, "Main.run");
		assert.strictEqual(
			typeof lockPayload.widgets[0].binding_fingerprint,
			"string",
		);

		const evidenceEntries =
			await vscode.workspace.fs.readDirectory(evidenceRoot);
		...
		const validationContent = Buffer.from(
			await vscode.workspace.fs.readFile(validationUri),
		).toString("utf8");
		const journeysContent = Buffer.from(
			await vscode.workspace.fs.readFile(journeysUri),
		).toString("utf8");
		...
		const secondLockContent = Buffer.from(
			await vscode.workspace.fs.readFile(lockUri),
		).toString("utf8");
		assert.strictEqual(
			secondLockContent,
	...
	test("LM HMI Phase 6 trace_capture writes scenario traces and preview_snapshot writes viewport artifacts", async () => {
		...
		__testSetRuntimeControlRequestHandler(
			async (_endpoint, _auth, requestType, params) => {
				...
				if (requestType === "hmi.values.get") {
					...
					const values: Record<
						string,
						{ v: unknown; q: string; ts_ms: number }
					> = {};
					for (const id of ids) {
						values[id] = {
							v: valueTick % 2 === 0,
		...
		const traceContent = Buffer.from(
			await vscode.workspace.fs.readFile(traceUri),
		).toString("utf8");
		...
		for (const file of previewPayload.files as Array<{ path: string }>) {
			...
			assert.ok(
				...
				`Expected SVG snapshot in ${file.path}`,
			);
		}
	});
	...
	test("LM HMI Phase 6 run_journey executes API/event flow and explain_widget reports provenance", async () => {
					...
					journeys: [
						{
							...
							steps: [
								{
									action: "read_values",
									ids: ["resource/resource/program/main/field/run"],
								},
								{
									action: "write",
		...
		const journeysUri = vscode.Uri.joinPath(
			hmiRoot,
			"_evidence",
			runId,
			"journeys.json",
		...
		const journeysPayload = JSON.parse(
			Buffer.from(await vscode.workspace.fs.readFile(journeysUri)).toString(
				"utf8",
			),
		);
		...
		assert.strictEqual(
			explainPayload.provenance.write_policy.allowlisted,
			false,
		);
	});
		...
		const patchResult = await patchTool.invoke(
			{
				input: {
					...
					operations: [
						{ op: "add", path: "/files/cancel.toml", value: 'title = "x"\n' },
					],
				},
			},
			tokenSource.token,
