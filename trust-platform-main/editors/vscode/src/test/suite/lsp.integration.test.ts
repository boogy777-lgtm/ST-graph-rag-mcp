...
suite("LSP integration (VS Code)", function () {
	...
	suiteSetup(async () => {
		...
		fixturesRoot = vscode.Uri.joinPath(workspaceFolder.uri, "tmp", "vscode-it");
		...
		await vscode.workspace
			...
			.update("enableTrash", false, vscode.ConfigurationTarget.Workspace);
		await delay(200);
	});
	...
	test("method calls surface VAR_INPUT completions and signature help", async () => {
		...
		const cursorOffset = source.indexOf("motor.Start(") + "motor.Start(".length;
		...
		const help = await waitForSignatureHelp(doc.uri, cursor, (value) =>
		...
		const labelsText = help.signatures
	...
	test("code actions surface undefined variable quick fix", async () => {
		...
		const collection =
			vscode.languages.createDiagnosticCollection("trust-lsp-test");
	...
	test("executeCommand relocates namespaces across files", async () => {
		...
		await delay(200);
		const targetContent = await waitForFileContent(targetUri, (text) =>
			text.includes("NAMESPACE Company.LibA"),
		);
		assert.ok(
		...
		const updatedMain = await waitForDocumentText(mainDoc, (text) =>
			text.includes("USING Company.LibA"),
		);
		assert.ok(
			...
			try {
				inlineAction = await waitForCodeAction(
					...
					(action) => {
						const title = action.title;
						return (
							typeof title === "string" && title.includes("Inline variable")
						);
					},
				);
			} catch {
