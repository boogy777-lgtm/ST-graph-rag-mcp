...
suite("Debug/IO DRY flows", function () {
	...
	suiteSetup(async () => {
		...
		originalSettings = __testCollectSettingsSnapshot() as Record<
			string,
			unknown
		>;
	});
	...
	test("integration: auto default configuration creation", async () => {
		const projectRoot = vscode.Uri.joinPath(
			fixturesRoot,
			"auto-config-project",
		);
		...
		const created = await __testCreateDefaultConfigurationAuto(
			"AutoConfigMain",
			mainUri,
		);
		...
		assert.ok(
			...
			"Expected created configuration file.",
		);
	...
	test("integration: settings update persists values", async () => {
		...
		try {
			...
			const snapshot = __testCollectSettingsSnapshot() as Record<
				string,
				unknown
			>;
	...
	test("integration: VM debug session returns non-empty stackTrace at stopOnEntry", async () => {
		...
		assert.ok(
			extension,
			"Expected trust-lsp extension to be installed for tests.",
		);
		await extension!.activate();
		const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
		assert.ok(
			workspaceFolder,
			"Expected workspace folder for debug session test.",
		);

		const projectRoot = vscode.Uri.joinPath(
			fixturesRoot,
			"stacktrace-session-project",
		);
		...
		try {
			...
			assert.ok(
				frames.length > 0,
				"Expected non-empty stackTrace while debugger is stopped.",
			);
