...
suite("New project command (VS Code)", function () {
	...
	suiteSetup(async () => {
		...
		assert.ok(
			...
			"Expected a workspace folder for tests.",
		);
	...
	test("creates scaffold in an empty target directory", async () => {
		...
		assert.strictEqual(
			await pathExists(mainUri),
			true,
			"Expected Main.st file.",
		);
		assert.strictEqual(
	...
	test("cancel at each prompt stage leaves filesystem unchanged", async () => {
		...
		const cancelAtNameTarget = vscode.Uri.joinPath(
			fixturesRoot,
			"cancel-at-name",
		);
		...
		const existingTarget = vscode.Uri.joinPath(
			fixturesRoot,
			"cancel-at-overwrite",
		);
	...
	test("existing target requires explicit confirmation behavior", async () => {
		...
		assert.strictEqual(
			accepted,
			true,
			"Expected explicit confirm to continue.",
		);
		assert.strictEqual(await pathExists(sentinelUri), true);
		assert.strictEqual(
			await pathExists(vscode.Uri.joinPath(targetUri, "src", "Main.st")),
	...
	test("works in single-root and multi-root workspace setups", async () => {
		...
		assert.strictEqual(
			await pathExists(
				vscode.Uri.joinPath(secondWorkspaceRoot, "hmi", hmiFile),
			),
			true,
		);
		assert.strictEqual(
			await pathExists(
				vscode.Uri.joinPath(workspaceFolder.uri, "hmi", hmiFile),
			),
			false,
		);
		...
		assert.ok(
			Array.isArray(layoutPayload.files) &&
				layoutPayload.files.some(
					(entry: { name?: string }) => entry.name === hmiFile,
				),
		);
	});
});
