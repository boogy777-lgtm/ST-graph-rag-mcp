...
function collectSettingsSnapshot(): SettingsPayload {
	...
	return {
		...
		runtimeControlAuthToken:
			config.get<string>("runtime.controlAuthToken") ?? "",
...
async function compileActiveProgram(
	...
	if (options.startDebugAfter) {
		if (errors > 0) {
			...
			statusMessage =
				"Save all Structured Text files before starting the runtime.";
		} else {
