...
async function handleNavigateMessage(payload: unknown): Promise<void> {
	...
	const widget = effectiveSchema.widgets.find(
	...
	setStatus(
		`Navigated to ${path.basename(location.uri.fsPath)}:${location.range.start.line + 1}`,
	);
}
...
async function handleSaveLayoutMessage(payload: unknown): Promise<void> {
	...
	try {
		...
		setStatus(
			`Saved HMI layout overrides (${Object.keys(parsed).length} widgets).`,
		);
...
async function loadProcessSvgContent(
	...
	const svgUri = vscode.Uri.joinPath(
		workspaceUri,
		"hmi",
		...normalized.split("/"),
	);
...
function normalizeProcessSvgPath(
...
export function __testSetControlRequestHandler(
...
export function __testValidateLayoutSavePayload(
	payload: unknown,
): LayoutOverrides {
...
export async function __testHandleTrustTwinInteraction(
	payload: unknown,
): Promise<void> {
