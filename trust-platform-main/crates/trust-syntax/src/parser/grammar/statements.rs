...
impl Parser<'_, '_> {
    ...
    fn parse_try_stmt(&mut self) {
        const MAX_TRY_DEPTH: u32 = 256;

        if self.try_depth >= MAX_TRY_DEPTH {
            self.error("__TRY nesting exceeds maximum depth");
            self.start_node(SyntaxKind::TryStmt);
            self.bump();
            while !self.at(TokenKind::KwEndTryDunder) && !self.at_end() && !self.at_stmt_list_end() {
                self.bump();
            }
        ...
        if self.at(TokenKind::KwFinallyDunder) {
            ...
            self.bump();
            while !self.at(TokenKind::KwEndTryDunder) && !self.at_end() {
                self.parse_statement();
            }
            self.finish_node();
        }
