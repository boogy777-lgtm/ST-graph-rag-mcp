...
impl Parser<'_, '_> {
    ...
    pub(crate) fn parse_var_block(&mut self) {
        let kind = if self.at(TokenKind::KwVarInst) {
            SyntaxKind::VarInstBlock
        } else if self.at(TokenKind::KwVarInOut) && self.peek_kind_n(1) == TokenKind::KwConstant {
        ...
        self.bump(); // VAR, VAR_INPUT, VAR_INST, VAR_IN_OUT, etc.

        // VAR_IN_OUT CONSTANT: consume the CONSTANT keyword
        if kind == SyntaxKind::VarInOutConstBlock {
            self.bump(); // CONSTANT (already confirmed by peek)
        }

        // Parse optional modifiers (retain, non_retain, persistent, constant for non-VAR_IN_OUT)
        while matches!(
            self.current(),
