use super::{lex_block_comment_c, lex_block_comment_pascal, lex_pragma};
use logos::Logos;
...
pub enum TokenKind {
    ...
    KwEndTryDunder,
    #[token("__QUERYINTERFACE", ignore(case))]
    KwQueryInterfaceDunder,
    #[token("__QUERYPOINTER", ignore(case))]
    KwQueryPointerDunder,
    #[token("__ISVALIDREF", ignore(case))]
    KwIsValidRefDunder,
    #[token("__VARINFO", ignore(case))]
    KwVarInfoDunder,
    #[token("__CURRENTTASK", ignore(case))]
    KwCurrentTaskDunder,
    #[token("__COMPARE_AND_SWAP", ignore(case))]
    KwCompareAndSwapDunder,
    #[token("__XADD", ignore(case))]
    KwXAddDunder,
    #[token("TEST_AND_SET", ignore(case))]
    KwTestAndSet,
    #[token("__VECTOR", ignore(case))]
    KwVectorDunder,
    #[token("__UXINT", ignore(case))]
    KwUXInt,
    #[token("__XWORD", ignore(case))]
    KwXWord,
    #[token("CAL", ignore(case))]
    KwCal,
