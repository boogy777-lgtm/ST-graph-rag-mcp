...
/// Checks if the position is inside a `__TRY` body (i.e., inside TryStmt
/// but not inside a CatchBlock or FinallyBlock).
pub(crate) fn is_inside_try_body(root: &SyntaxNode, position: TextSize) -> bool {
    let Some(token) = root.token_at_offset(position).right_biased() else {
        return false;
    };

    for ancestor in token.parent_ancestors() {
        match ancestor.kind() {
            SyntaxKind::CatchBlock | SyntaxKind::FinallyBlock => return false,
            SyntaxKind::TryStmt => return true,
            _ => {}
        }
    }
    false
