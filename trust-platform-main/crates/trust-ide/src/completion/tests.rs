...
"#;
    ...
    #[test]
    fn test_completion_new_keywords() {
        let source = r#"
PROGRAM Main
VAR
    x : INT;
...
"#;
        ...
        let expect_keywords = [
            "__TRY",
            "CAL", "__NEW", "__DELETE", "__ISVALIDREF",
            "AND_THEN", "OR_ELSE",
        ];
        for kw in &expect_keywords {
            assert!(
                items.iter().any(|item| item.label.eq_ignore_ascii_case(kw)),
                "completion should include keyword: {kw}",
            );
        }

        assert!(
            items.iter().any(|item| item.label.eq_ignore_ascii_case("VAR_INST")),
            "completion should include VAR_INST",
        );
    }

    #[test]
    fn test_completion_try_body_offers_catch_finally_endtry() {
        let source = r#"
PROGRAM Main
VAR
    x : INT;
...
"#;
        ...
        let items = complete(&db, file_id, TextSize::from(cursor as u32));
        assert!(
            items.iter().any(|item| item.label == "__CATCH"),
            "completion inside __TRY body should offer __CATCH"
        );
        assert!(
            items.iter().any(|item| item.label == "__FINALLY"),
            "completion inside __TRY body should offer __FINALLY"
        );
        assert!(
            items.iter().any(|item| item.label == "__ENDTRY"),
            "completion inside __TRY body should offer __ENDTRY"
        );
    }
}
