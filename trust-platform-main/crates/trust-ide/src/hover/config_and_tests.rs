...
"#;
        ...
        let result = hover(&db, file_id, TextSize::from(cursor as u32));
        assert!(
            result.is_some(),
            "hover on ptr->field should return a result"
        );
        let hover_result = result.unwrap();
        assert!(
            hover_result.contents.contains("field") || hover_result.contents.contains("INT"),
            "hover should show field info: got '{}'",
            hover_result.contents
        );
    }

    #[test]
    fn test_hover_arrow_deref_explicit() {
        let source = r#"
TYPE MyStruct :
STRUCT
    field : INT;
...
"#;
        ...
        let hover_result = result.unwrap();
        assert!(
            hover_result.contents.contains("FIELD") || hover_result.contents.contains("field"),
            "hover should show field info: got '{}'",
            hover_result.contents
        );
    }
    ...
    #[test]
    fn test_hover_slice_expr() {
        let source = r#"
PROGRAM Main
VAR
    arr : ARRAY[0..9] OF INT;
    sub : ARRAY[0..4] OF INT;
END_VAR
    sub := ar|r[1..5];
END_PROGRAM
"#;
        ...
        let result = hover(&db, file_id, TextSize::from(cursor as u32)).expect("hover result");
        assert!(
            result.contents.contains("ARRAY"),
            "hover on arr should show array type: got '{}'",
            result.contents
        );
    }
