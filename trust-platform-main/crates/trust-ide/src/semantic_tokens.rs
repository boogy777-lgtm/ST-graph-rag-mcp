...
pub fn semantic_tokens(db: &Database, file_id: FileId) -> Vec<SemanticToken> {
    ...
    for token in tokens {
        let semantic_type = match token.kind {
            ...
            | TokenKind::KwQueryInterfaceDunder
            | TokenKind::KwQueryPointerDunder
            | TokenKind::KwIsValidRefDunder
            | TokenKind::KwVarInfoDunder
            | TokenKind::KwCurrentTaskDunder
            | TokenKind::KwCompareAndSwapDunder
            | TokenKind::KwXAddDunder
            | TokenKind::KwTestAndSet
            | TokenKind::KwVectorDunder
            | TokenKind::KwUXInt
            | TokenKind::KwXWord
            | TokenKind::KwNew
            ...
            | TokenKind::KwOverride => Some(SemanticTokenType::Keyword),
            ...
            | TokenKind::Ampersand
            | TokenKind::SetAssign
            | TokenKind::ResetAssign
            | TokenKind::ArrowDeref => Some(SemanticTokenType::Operator),
...
"#;
        ...
        let tokens = semantic_tokens(&db, file_id);

        let try_token = tokens
            .iter()
            .find(|t| &source[t.range] == "__TRY")
            .expect("__TRY should have a semantic token");
        assert_eq!(
            try_token.token_type,
            SemanticTokenType::Keyword,
            "__TRY should be highlighted as Keyword"
        );

        let catch_token = tokens
            .iter()
            .find(|t| &source[t.range] == "__CATCH")
            .expect("__CATCH should have a semantic token");
        assert_eq!(
            catch_token.token_type,
            SemanticTokenType::Keyword,
            "__CATCH should be highlighted as Keyword"
        );

        let finally_token = tokens
            .iter()
            .find(|t| &source[t.range] == "__FINALLY")
            .expect("__FINALLY should have a semantic token");
        assert_eq!(
            finally_token.token_type,
            SemanticTokenType::Keyword,
            "__FINALLY should be highlighted as Keyword"
        );

        let endtry_token = tokens
            .iter()
            .find(|t| &source[t.range] == "__ENDTRY")
            .expect("__ENDTRY should have a semantic token");
        assert_eq!(
            endtry_token.token_type,
            SemanticTokenType::Keyword,
            "__ENDTRY should be highlighted as Keyword"
        );
    }
}
