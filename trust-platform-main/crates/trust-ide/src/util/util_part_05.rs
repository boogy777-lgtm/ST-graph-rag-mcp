...
fn resolve_field_target(
    ...
    let is_arrow = parent.kind() == SyntaxKind::ArrowExpr;
    let is_field = parent.kind() == SyntaxKind::FieldExpr;
    if !is_arrow && !is_field {
        return None;
    }

    let base_expr = parent.children().next()?;
    let base_type = expression_type_at_node(db, file_id, &base_expr)?;
    let base_type = symbols.resolve_alias_type(base_type);

    let resolved_type = if is_arrow {
        match symbols.type_by_id(base_type)? {
            Type::Pointer { target } | Type::Reference { target } => {
                symbols.resolve_alias_type(*target)
            }
            _ => base_type,
        }
    } else {
    ...
    if let Some(member_id) =
        symbols.resolve_member_symbol_in_type(resolved_type, field_name)
    {
        return Some(ResolvedTarget::Symbol(member_id));
    }

    if let Some(field_target) = resolve_struct_field(symbols, resolved_type, field_name) {
