...
impl<'a, 'b> ExprChecker<'a, 'b> {
    ...
    pub(crate) fn check_expression(&mut self, node: &SyntaxNode) -> TypeId {
        let type_id = match node.kind() {
            ...
            SyntaxKind::ArrowExpr => self.infer_arrow_expr(node),
            SyntaxKind::SliceExpr => self.infer_slice_expr(node),
            SyntaxKind::NewExpr => self.infer_new_expr(node),
            SyntaxKind::DeleteExpr => self.infer_delete_expr(node),
            _ => self
                .checker
    ...
    /// Infers the type of an arrow expression (`ref->member`).
    ///
    /// `ref->member` is syntactic sugar for `(*ref).member`. Per IEC 61131-3
    /// extension semantics, the left operand must be a reference or pointer type,
    /// and the right operand is a member of the target type.
    fn infer_arrow_expr(&mut self, node: &SyntaxNode) -> TypeId {
        let children: Vec<_> = node.children().collect();
        if children.len() < 2 {
            return self
                .checker
        ...
        if base_type == TypeId::UNKNOWN || resolved_base == TypeId::UNKNOWN {
            return self
                .checker
                .legacy_suppressed_type(DiagnosticCode::CannotResolve, base.text_range());
        }

        let target = match self.checker.symbols.type_by_id(resolved_base) {
            Some(Type::Pointer { target } | Type::Reference { target }) => *target,
            _ => {
                return self.checker.legacy_diagnostic_type(
                    DiagnosticCode::TypeMismatch,
                    base.text_range(),
                    "arrow dereference requires a pointer or reference type",
                );
            }
        };

        let field_name = match self.checker.resolve_ref().get_name_from_ref(member) {
            Some(n) => n,
            None => {
                return self
                    .checker
                    .legacy_type_from_outcome(self.checker.unknown_type_outcome(member.text_range()));
            }
        };
        ...
        if let Some(field_type) = self
            .checker
            .resolve_ref()
            .resolve_member_in_type(target, field_name.as_str())
        {
            return field_type;
        }
        ...
        self.checker.legacy_diagnostic_type(
            ...
            member.text_range(),
            format!(
                "no member '{}' on dereferenced type '{}'",
                field_name,
                self.checker.type_name(target)
            ),
        )
    }

    /// Infers the type of a slice expression (`arr[lo..hi]`).
    ///
    /// Per IEC 61131-3 extension semantics, a slice takes a subrange of
    /// an array dimension and produces a view of the same array type.
    /// Both lower and upper bound must be integer expressions.
    fn infer_slice_expr(&mut self, node: &SyntaxNode) -> TypeId {
        let children: Vec<_> = node.children().collect();
        if children.len() < 3 {
            return self
                .checker
        ...
        for idx in [&lo, &hi] {
            ...
            let resolved_idx = self.checker.resolve_alias_type(idx_ty);
            if resolved_idx != TypeId::UNKNOWN {
                if let Some(ty) = self.checker.symbols.type_by_id(resolved_idx) {
                    if !ty.is_integer() {
                        self.checker.diagnostics.error(
                            DiagnosticCode::TypeMismatch,
                            idx.text_range(),
                            "slice index must be integer type",
                        );
                    }
                }
            }
        ...
        match self.checker.symbols.type_by_id(resolved_base) {
            Some(Type::Array { element, .. }) => *element,
            Some(Type::String { .. }) => TypeId::CHAR,
            Some(Type::WString { .. }) => TypeId::WCHAR,
            _ => self.checker.legacy_diagnostic_type(
                DiagnosticCode::TypeMismatch,
                node.text_range(),
                "slice requires an array or string type",
            ),
        }
    }

    /// Infers the type of a dynamic allocation expression (`__NEW(T, n)`).
    ///
    /// Per IEC 61131-3 extension semantics, `__NEW` dynamically allocates
    /// `n` elements of type `T` and returns `POINTER TO T`. The type `T`
    /// must exist in the symbol table, and `n` (if present) must be `DINT`.
    fn infer_new_expr(&mut self, node: &SyntaxNode) -> TypeId {
        ...
        for child in node.children() {
            if child.kind() == SyntaxKind::TypeRef {
                type_ref = Some(child);
            } else if child.kind().is_expression_node() {
                size_expr = Some(child);
            }
        }

        let Some(type_node) = type_ref else {
            return self.checker.legacy_diagnostic_type(
                DiagnosticCode::UndefinedType,
                node.text_range(),
                "__NEW requires a type argument",
            );
        };
        ...
        let target_type = match self
            .checker
            .resolve_ref()
            .resolve_type_from_expr_outcome(&type_node)
        {
            crate::semantic::SemanticOutcome::Resolved(type_id) => type_id,
            crate::semantic::SemanticOutcome::Unknown { .. } => {
                return self.checker.legacy_diagnostic_type(
                    DiagnosticCode::UndefinedType,
                    node.text_range(),
                    "__NEW expects a valid type name",
                );
            }
            _ => {
                return self.checker.legacy_diagnostic_type(
        ...
        if let Some(size) = &size_expr {
            ...
            let resolved_size = self.checker.resolve_alias_type(size_type);
            if resolved_size != TypeId::DINT && resolved_size != TypeId::UNKNOWN {
                if let Some(ty) = self.checker.symbols.type_by_id(resolved_size) {
                    if !ty.is_integer() {
                        self.checker.diagnostics.error(
                            DiagnosticCode::TypeMismatch,
                            size.text_range(),
                            "__NEW size must be DINT (or integer) type",
                        );
                    }
                }
            }
        ...
        self.checker
            .symbols
            .register_pointer_type(target_type)
    }

    /// Infers the type of a deallocation expression (`__DELETE(p)`).
    ///
    /// Per IEC 61131-3 extension semantics, `__DELETE` deallocates memory
    /// previously allocated with `__NEW`. The argument must be a pointer or
    /// reference type, or NULL. Returns VOID.
    fn infer_delete_expr(&mut self, node: &SyntaxNode) -> TypeId {
        let arg = match node.children().next() {
            Some(child) => child,
            None => {
                return self.checker.legacy_diagnostic_type(
                    DiagnosticCode::WrongArgumentCount,
                    node.text_range(),
                    "__DELETE requires an argument",
                );
            }
        };
        ...
        if resolved == TypeId::UNKNOWN {
            return self
                .checker
                .legacy_suppressed_type(DiagnosticCode::CannotResolve, arg.text_range());
        }

        match self.checker.symbols.type_by_id(resolved) {
            Some(Type::Pointer { .. } | Type::Reference { .. }) | Some(Type::Null) => {}
            _ => {
                self.checker.diagnostics.error(
                    DiagnosticCode::InvalidArgumentType,
                    arg.text_range(),
                    "__DELETE requires a pointer, reference, or NULL argument",
                );
            }
        }
