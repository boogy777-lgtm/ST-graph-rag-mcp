    ...
    match stmt {
        ...
        Stmt::Try {
            ...
            let catch_result = match result {
                Ok(StmtResult::Continue) => None,
                Err(_err) => {
                    if let Some(var_name) = catch_var {
                        let placeholder = Value::Null;
                        write_lvalue(ctx, &LValue::Name(var_name.clone()), placeholder)?;
                    }
                    Some(exec_block(ctx, catch_body)?)
                }
                other => Some(other?),
            };
            let finally_result = exec_block(ctx, finally_body)?;
            if let Some(catch_res) = catch_result {
                match catch_res {
                    StmtResult::Continue => {}
                    _ => return Ok(catch_res),
                }
            }
