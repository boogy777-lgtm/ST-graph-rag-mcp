impl<'a> BytecodeEncoder<'a> {
    ...
    fn emit_try_stmt(
        ...
        pou_id: u32,
        body: &[crate::program_model::Stmt],
        _catch_body: &[crate::program_model::Stmt],
        _finally_body: &[crate::program_model::Stmt],
        code: &mut Vec<u8>,
        debug_entries: &mut Vec<DebugEntry>,
    ) -> Result<bool, BytecodeError> {
        ctx.warnings.push(
            "W1001: __TRY/__CATCH/__FINALLY runtime execution not yet supported. The try body will be executed as normal statements; no exception handling occurs."
                .into(),
        );

        self.emit_block(ctx, pou_id, body, code, debug_entries)?;
...
fn stmt_contains_c1_required_call(stmt: &crate::program_model::Stmt) -> bool {
    ...
    match stmt {
        ...
        Stmt::Try {
            ...
            body.iter().any(stmt_contains_c1_required_call)
                || catch_body.iter().any(stmt_contains_c1_required_call)
                || finally_body.iter().any(stmt_contains_c1_required_call)
        }
...
fn stmt_contains_c5_required_construct(stmt: &crate::program_model::Stmt) -> bool {
    ...
    match stmt {
        ...
        Stmt::Try {
            ...
            body.iter().any(stmt_contains_c5_required_construct)
                || catch_body.iter().any(stmt_contains_c5_required_construct)
                || finally_body.iter().any(stmt_contains_c5_required_construct)
        }
    }
}
